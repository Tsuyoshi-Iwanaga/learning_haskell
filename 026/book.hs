...
import qualified Data.ByteString as B --Byte
import qualified Data.Text as T -- Unicode
...
getRecordLength leader = rawToInt (B.take 5 leader) -- 先頭5バイトがレコードの長さを表す
...
getBaseAddress leader = rawToInt (B.take 5 remainder)
  where remainder = B.drop 12 leader -- リーダーの12文字目から16番目のバイト(5バイト)がベースアドレスの情報
...
-- ディレクトリのエントリを処理し、MARCレコードのフィールドを調べる
